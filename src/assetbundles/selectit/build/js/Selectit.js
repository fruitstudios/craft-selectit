...
var Selectit = (function() {
	...
	var defaults = {
		...
		sortable: {
			animation: 150,
			// handle: ".selectit--handle",
			draggable: ".selectit--asset",
			dragClass: "selectit--assetDragging",
			ghostClass: "selectit--assetGhost",
			chosenClass: "selectit--assetChosen",
			filter: ".selectit--controls, .selectit--remove",
		},
		layout: "grid",
		preview: "image",
		csrfTokenName: "",
	...
	var templates = {
		// placeholder: '<li class="selectit--placeholder selectit--isLoading"><span class="selectit--placeholderCancel">Cancel</span><span class="selectit--placeholderProgress"></span><span class="selectit--placeholderError"></span></li>'
		placeholder: '<li class="selectit--placeholder selectit--isLoading"><span class="selectit--placeholderCancel"><a class="selectit--remove"><svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><path d="M32.202 27.36L58.62.944c1.226-1.225 3.212-1.225 4.437 0s1.225 3.21 0 4.437L36.64 31.8l26.417 26.418c1.225 1.225 1.225 3.212 0 4.437s-3.21 1.225-4.437 0l-26.418-26.42L5.784 62.653c-1.225 1.225-3.212 1.225-4.437 0s-1.225-3.212 0-4.437l26.418-26.418L1.347 5.38C.122 4.154.122 2.168 1.347.943s3.212-1.225 4.437 0L32.202 27.36z"/></svg></a></span><span class="selectit--placeholderProgress"></span><span class="selectit--placeholderError"></span></li>'
	};
	...
	var constructor = function(options) {
		...
		var checkLimit = function() {
			...
			if (settings.limit && numberOfUploadedAssets >= settings.limit) {
				dom.controls.classList.add("selectit--isHidden");
			} else {
				dom.controls.classList.remove("selectit--isHidden");
			}
		};
		...
		var dropToUploadHandler = function(event) {
			var upload = event.target.closest(".selectit--uploader");
			if (!upload) {
				return;
			}
			...
			switch (event.type) {
				case "dragenter":
				case "dragover":
					upload.classList.add("selectit--isDragging");
					break;

				case "dragleave":
					upload.classList.remove("selectit--isDragging");
					break;
		...
		var removeAssetHandler = function(event) {
			var asset = event.target.closest(".selectit--asset");
			if (!asset) {
				return;
			}
			event.preventDefault();
		...
		var uploadAssetHandler = function(event) {
			if (!event.target.closest(".selectit--upload")) {
				return;
			}
			event.preventDefault();
			dom.input.click();
		...
		var assetInputHandler = function(event) {
			if (!event.target.closest('[name="selectitAssetInput"]')) {
				return;
			}
			event.preventDefault();
		...
		var cancelUploadHandler = function(event) {
			if (!event.target.closest('.selectit--placeholderCancel')) {
				return;
			}
			event.preventDefault();
			var placeholder = event.target.closest('.selectit--placeholder');
		...
		var setUploadError = function(qid, error) {
			queue[qid].dom.placeholder.classList.remove('selectit--isLoading');
			...
			assets.forEach(function(asset, i) {
				...
				queue[qid] = {
					...
					dom: {
						...
						error: placeholder.querySelector('.selectit--placeholderError'),
						progress: placeholder.querySelector('.selectit--placeholderProgress'),
					}
				};
		...
		api.canUploadAssets = function(success) {
			...
			formData.append("action", "selectit/upload/can-upload");
		...
		var getAssetFormData = function(asset) {
			var formData = new FormData();
			formData.append("action", "selectit/upload");
		...
		var preloadImage = function(url, success) {
	        ...
	        var image = htmlToElement('<img style="display:none !important;" class="selectit--isHidden" src="'+url+'">');
		...
		api.setGlobalError = function(error) {
			...
			if (error) {
				...
				dom.errors.classList.remove("selectit--isHidden");
			} else {
				dom.errors.textContent = "";
				dom.errors.classList.add("selectit--isHidden");
			}
		};
		...
		api.init = function(options) {
			...
			dom.assets = dom.uploader.querySelector(".selectit--assets");
			dom.controls = dom.uploader.querySelector(".selectit--controls");
			dom.input = dom.uploader.querySelector(
				'[name="selectitAssetInput"]'
			);
			dom.errors = dom.uploader.querySelector(".selectit--errors");
			dom.preload = dom.uploader.querySelector(".selectit--preload");
